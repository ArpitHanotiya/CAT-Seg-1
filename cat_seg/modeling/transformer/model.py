...
class CATSeg(nn.Module):
    def __init__(self, cfg):
        super(CATSeg, self).__init__()
        self.backbone = build_model(cfg).backbone
        self.sem_seg_head = build_model(cfg).sem_seg_head
        # Add temporal convolution
        self.temporal_conv = nn.Conv3d(
            in_channels=cfg.MODEL.SEM_SEG_HEAD.IN_CHANNELS,
            out_channels=cfg.MODEL.SEM_SEG_HEAD.IN_CHANNELS,
            kernel_size=(3, 1, 1),
            padding=(1, 0, 0)
        )
    ...
    def forward(self, batched_inputs):
        ...
        images = torch.stack(images)  # Shape: (batch, channels, time_steps, height, width)
        
        # Process time series
        images = self.temporal_conv(images)  # Shape: (batch, channels, time_steps, height, width)
        images = images.mean(dim=2)  # Aggregate time steps: (batch, channels, height, width)
        
        features = self.backbone(images)
        outputs = self.sem_seg_head(features)
        return outputs

# Update build_model to use CATSeg
def build_cat_seg_model(cfg):
    model = CATSeg(cfg)
    model.device = torch.device(cfg.MODEL.DEVICE)
    return model